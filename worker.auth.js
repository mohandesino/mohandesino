  ...
  async fetch(request, env) {
    ...
    if (path === "/api/admin/me" && request.method === "POST") {
      ...
      if (courseMatch && request.method === "GET") {
        ...
        let hasAccess = Number(course.is_free) === 1;

        if (!hasAccess && currentUser) {
          const enrollment = await env.DB.prepare(
            "SELECT id FROM enrollments WHERE user_id = ? AND course_id = ? LIMIT 1",
          )
            .bind(currentUser.id, courseId)
            .first();
        ...
        for (const chapter of chapters) {
          const lessonQuery = hasAccess
            ? `SELECT * FROM lessons
               WHERE chapter_id = ?
               ORDER BY sort_order ASC, id ASC`
            : `SELECT * FROM lessons
               WHERE chapter_id = ? AND free = 1
               ORDER BY sort_order ASC, id ASC`;

          const { results: lessons } = await env.DB.prepare(lessonQuery)
            .bind(chapter.id)
            .all();
